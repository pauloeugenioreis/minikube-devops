...
run_setup() {
    ...
    local setup_script="$PROJECT_PATH/scripts/linux/setup-fresh-machine.sh"
    if [[ ! -f "$setup_script" ]]; then
        setup_script="$PROJECT_PATH/minikube/scripts/linux/setup-fresh-machine.sh"
    fi
    if [[ -f "$setup_script" ]]; then
        ...
        echo "  bash scripts/linux/setup-fresh-machine.sh --run-initialization"
        return 1
    fi
}
...
print_final_status() {
    ...
    if [[ "$SKIP_SETUP" == "true" ]]; then
        ...
        echo "  cd $PROJECT_PATH"
        echo "  bash ${cmd_prefix}scripts/linux/setup-fresh-machine.sh --run-initialization"
        echo ""
    fi
