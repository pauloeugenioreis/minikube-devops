...
function Get-LayoutBasePath {
    param([string]$Path)

    $legacyBase = Join-Path $Path "minikube"
    if (Test-Path (Join-Path $legacyBase "scripts\windows") -and Test-Path (Join-Path $legacyBase "charts")) {
        return $legacyBase
    }

    if (Test-Path (Join-Path $Path "scripts\windows") -and Test-Path (Join-Path $Path "charts")) {
        return $Path
    }
...
function Test-ProjectStructure {
    ...
    return $null -ne (Get-LayoutBasePath -Path $Path)
}
...
function Run-Setup {
    ...
    $layoutBase = Get-LayoutBasePath -Path $ProjectPath
    if (-not $layoutBase) {
        Write-Error "Estrutura do projeto nao reconhecida em: $ProjectPath"
        throw "Invalid project structure"
    }

    $setupScript = Join-Path $layoutBase "scripts\windows\Setup-Fresh-Machine.ps1"
...
function Show-FinalInstructions {
    $layoutBase = Get-LayoutBasePath -Path $ProjectPath
    $cmdPrefix = ""
    if ($layoutBase -and $layoutBase -ne $ProjectPath) {
        $cmdPrefix = "minikube\\"
    }
